...
1. **Check processed task tracking**: Read **only your own** `## team={your team_id}` section of `session_processed_tasks.md`, in the agent-memory directory the platform gave you, for already-processed task IDs — the **UNION of every `Processed task IDs` line** in that section, never the newest line alone. **Exception: a task the Standard Harvest Step 3 quiescent-task rule sent you back for enters this pass's population even though the ledger lists it as processed.** That widens the population, which is always permitted; only narrowing it is forbidden
